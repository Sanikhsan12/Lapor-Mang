...
const UserDashboard = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setLoading(false);
      setTimeout(() => { if(msg.type === 'success') setMsg({ type: '', text: '' }); }, 3000);
    }
  };
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case 'under investigation': return 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400';
    ...
    <div className="min-h-screen bg-bg-light dark:bg-bg-dark pt-8 px-4 pb-20">
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="flex justify-between items-center bg-surface-light dark:bg-surface-dark p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
          <div>
            <h1 className="text-2xl font-bold text-slate-800 dark:text-white">Welcome, {user.username}</h1>
            <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Submit and track your health reports seamlessly</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button onClick={logoutAuth} className="flex items-center gap-2 text-red-500 hover:text-red-600 transition-colors bg-red-50 dark:bg-red-900/10 p-2 px-4 rounded-lg font-medium">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="card-surface flex items-center gap-4 hover:-translate-y-1 transition-transform">
            ...
            <div>
              <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Total Reports</p>
          ...
          <div className="card-surface flex items-center gap-4 hover:-translate-y-1 transition-transform">
            ...
            <div>
              <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Pending Processing</p>
        ...
        <div className="grid md:grid-cols-5 gap-8">
          ...
          <div className="card-surface md:col-span-2">
            <div className="flex items-center gap-2 mb-6 border-b border-slate-100 dark:border-slate-700 pb-4">
              ...
              <h2 className="text-xl font-bold text-slate-800 dark:text-white">New Report</h2>
            </div>
            
            {msg.text && (
              <div className={`p-4 rounded-lg mb-6 text-sm flex items-center gap-2 border ${
                msg.type === 'error' ? 'bg-red-50 text-red-700 border-red-200 dark:bg-red-900/20 dark:border-red-800 dark:text-red-400' : 
                msg.type === 'info' ? 'bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-900/20 dark:border-blue-800 dark:text-blue-400' :
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">Opsional — membantu admin menghubungi kamu lebih cepat</p>
              </div>
              <div>
                <label className="block text-sm font-semibold mb-1 text-slate-700 dark:text-slate-300">Photo Evidence</label>
                  ...
                  <div className="space-y-2 text-center">
                    ...
                    <div className="flex text-sm text-slate-600 dark:text-slate-400 justify-center">
                      <label className="relative cursor-pointer rounded-md font-semibold text-primary hover:text-primary-dark focus-within:outline-none focus-within:ring-2 inline-block">
                        ...
                        <input id="file-upload" name="file-upload" type="file" className="sr-only" onChange={handlePhotoChange} accept="image/*" required/>
                      </label>
                    </div>
                    <p className="text-xs text-slate-500">Auto-compressed format support</p>
                    {photo && <p className="text-sm font-bold text-primary mt-3 bg-primary/10 py-1 px-2 rounded-md inline-block">{photo.name}</p>}
                  </div>
                </div>
              </div>
              <button disabled={loading} type="submit" className="w-full btn-primary py-3 text-lg mt-2 font-bold shadow-lg shadow-primary/30">
          ...
          <div className="card-surface flex flex-col md:col-span-3">
            <h2 className="text-xl font-bold mb-6 text-slate-800 dark:text-white border-b border-slate-100 dark:border-slate-700 pb-4">Recent Submissions</h2>
            <div className="space-y-4 flex-1 overflow-y-auto pr-2 custom-scrollbar" style={{maxHeight: '680px'}}>
              {reports.length === 0 ? (
                <div className="flex flex-col items-center justify-center text-center text-slate-500 mt-20 space-y-3">
                  ...
                  <p className="text-sm text-slate-400">Use the form to create your first report.</p>
                </div>
              ) : (
                reports.map(r => (
                  <div key={r.id} className="rounded-xl border border-slate-200 dark:border-slate-700 hover:border-primary/50 bg-white dark:bg-surface-dark transition-all hover:shadow-md overflow-hidden">
                    {/* Card Header */}
                    <div className="p-5">
                      <div className="flex justify-between items-start mb-3">
                        <h3 className="font-bold text-lg text-slate-800 dark:text-white line-clamp-1 pr-4">{r.title}</h3>
                        <span className={`text-[10px] px-3 py-1 rounded-full uppercase font-bold tracking-wider ${getStatusColor(r.status)} shrink-0`}>
                          {r.status}
                        </span>
                      </div>
                      <p className="text-sm text-slate-600 dark:text-slate-400 mb-3 bg-slate-50 dark:bg-slate-800/50 p-3 rounded-lg border border-slate-100 dark:border-slate-800">{r.description}</p>
                      
                      {/* Meta info */}
                      <div className="flex flex-wrap gap-3 text-xs text-slate-500 dark:text-slate-400 mb-3">
                        <span className="flex items-center gap-1"><Clock size={13}/> {new Date(r.createdAt).toLocaleDateString()} at {new Date(r.createdAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        >
                          {expandedId === r.id ? <><ChevronUp size={14}/> Tutup Detail</> : <><ChevronDown size={14}/> Lihat Balasan Admin</>}
                        </button>
                      </div>
                    </div>
                    ...
                    {expandedId === r.id && (
                      <div className="border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/40 p-4">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="text-sm font-semibold text-slate-700 dark:text-slate-300">Balasan dari Admin</span>
                        </div>
                        {r.adminResponse ? (
                          ...
                          <p className="text-sm text-slate-400 dark:text-slate-500 italic">
                            Belum ada balasan dari admin. Harap tunggu — laporan kamu sedang diproses.
                          </p>
                        )}
                      </div>
                    )}
